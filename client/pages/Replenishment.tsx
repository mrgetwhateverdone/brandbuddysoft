...
export default function Replenishment() {
  ...
  const loadReplenishmentData = async () => {
    try {
      ...
      const alerts: ReplenishmentAlert[] = Array.from(skuMap.values())
        .map((item) => {
          ...
          const daysUntilStockout =
            currentStock > 0 ? Math.floor(currentStock / dailyVelocity) : 0;

          // Check for inbound POs for this SKU
          const relatedInbound = inbound.filter(
            (po) => po.sku === item.product_sku,
          );
          const inboundPOs = relatedInbound.length;
          const nextArrival =
            relatedInbound.length > 0
              ? relatedInbound[0].expected_arrival_date
              : "";
        ...
        .filter(
          (alert) => alert.risk_level !== "low" || alert.daysUntilStockout < 21,
        );
      ...
      // Calculate metrics
      const criticalSKUs = alerts.filter(
      ...
      const totalPOsOpen = inbound.filter(
      ...
      const generatedInsights =
        await openaiService.generateReplenishmentInsights(inbound, inventory);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Forecast-driven risk detection for stockouts and supply chain lag
          </p>
        </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {metrics.criticalSKUs.toLocaleString()}
                </p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {metrics.delayedInbounds.toLocaleString()}
                </p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {metrics.totalPOsOpen.toLocaleString()}
                </p>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Estimated revenue impact from potential stockouts
              </p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-bold text-destructive">
                ${metrics.totalValueAtRisk.toLocaleString()}
              </p>
              <p className="text-sm text-muted-foreground">
                {metrics.stockoutRisk} SKUs at risk
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            SKUs requiring immediate attention for replenishment
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <tbody>
                {alerts.slice(0, 20).map((alert) => (
                  <tr key={alert.sku} className="border-b">
                    ...
                    <td className="p-2">
                      <span
                        className={`font-medium ${alert.daysUntilStockout < 7 ? "text-destructive" : "text-foreground"}`}
                      >
          ...
          <CardContent className="pt-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Connect to Tinybird in Settings to view real replenishment
                  data.
                </p>
              </div>
            </div>
