    ...
    <div className="h-16 border-b border-border bg-background px-6 flex items-center justify-between">
      <div className="flex items-center space-x-4">
        <h2 className="text-lg font-semibold">
          Brand Intelligence Decision Engine
        </h2>
      </div>
          ...
          <SheetContent className="w-96">
            ...
            <div className="mt-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {alerts
                    ...
                    .map((alert) => (
                      <div
                        key={alert.id}
                        className="text-sm p-2 bg-warning/10 rounded cursor-pointer"
                        onClick={() => onAlertClick(alert)}
                      >
                        {alert.title}
                      </div>
              ...
              {alerts.filter((a) => a.type === "inventory").length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium text-sm text-info">
                    Inventory Alerts
                  </h4>
                  {alerts
                    ...
                    .map((alert) => (
                      <div
                        key={alert.id}
                        className="text-sm p-2 bg-info/10 rounded cursor-pointer"
                        onClick={() => onAlertClick(alert)}
                      >
                        {alert.title}
                      </div>
              ...
              {alerts.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-xs">
                    Decision engine monitoring all systems
                  </p>
                </div>
              )}
            </div>
